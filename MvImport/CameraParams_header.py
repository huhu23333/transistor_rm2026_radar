...
V_Undefined = 99                                 # < \~chinese 未定义                          \~english  Object is not yet initialized
V_Invisible = 3                                  # < \~chinese 不可见                          \~english  Not Visible
V_Guru = 2                                       # < \~chinese 大师可见                        \~english  Visible for Gurus
V_Expert = 1                                     # < \~chinese 专家可见                        \~english Visible for experts or Gurus
...
V_Beginner = 0                                   # < \~chinese 总是可见                          \~english  Always visible
int8_t = c_int8
int16_t = c_int16
int32_t = c_int32
int64_t = c_int64
...
_MV_FRAME_OUT_INFO_._fields_ = [
    ...
    ('nFrameLen', c_uint),             # < \~chinese 帧的长度               \~english The Length of Frame
    ('nLostPacket', c_uint),           # < \~chinese 本帧丢包数     \~english the number of lost packets
    ('nReserved', c_uint * 2),         # < \~chinese 保留字节           \~english Reserved bytes
]
...
_MV_DISPLAY_FRAME_INFO_._fields_ = [
    ...
    ('enRenderMode', c_uint),         # < \~chinese 图像渲染方式 0-默认模式(OPENGL)  \~english Render mode 0-Default OpenGL Mode
...
_MV_SAVE_IMAGE_PARAM_T_._fields_ = [
    ...
    ('enPixelType', MvGvspPixelType),  # < \~chinese 输入像素格式      \~english Input Data Pixel Format
    ('nWidth', c_ushort),              # < \~chinese 图像宽            \~english Image Width
    ('nHeight', c_ushort),             # < \~chinese 图像高            \~english Image Height
...
_MV_IMAGE_BASIC_INFO_._fields_ = [
    ('nWidthValue', c_ushort),  # < \~chinese 宽度           \~english width
    ('nWidthMin', c_ushort),    # < \~chinese 宽度最小值      \~english min width
    ('nWidthMax', c_uint),      # < \~chinese 宽度最大值      \~english max width
    ('nWidthInc', c_uint),      # < \~chinese 宽度           \~english width
    ('nHeightValue', c_uint),   # < \~chinese 高度           \~english height
    ('nHeightMin', c_uint),     # < \~chinese 高度最小值      \~english min height
    ('nHeightMax', c_uint),     # < \~chinese 高度最大值      \~english max height
    ('nHeightInc', c_uint),     # < \~chinese 高度           \~english height
    ('fFrameRateValue', c_float),  # < \~chinese 帧率         \~english frame rate
    ('fFrameRateMin', c_float),    # < \~chinese 帧率最小值     \~english min frame rate
    ('fFrameRateMax', c_float),   # < \~chinese 帧率最大值      \~english max frame rate
    ('enPixelType', c_uint),      # < \~chinese 当前的像素格式    \~english Current pixel format
    ('nSupportedPixelFmtNum', c_uint),  # < \~chinese 支持的像素格式种类    \~english Support pixel format
    ('enPixelList', c_uint * 64),       # < \~chinese 像素格式列表         \~english pixel format list
    ('nReserved', c_uint * 8),          # < \~chinese 保留字节            \~english Reserved bytes
]
...
_MV_XML_NODE_FEATURE_._fields_ = [
    # CameraParams.h 548
    ('enType', MV_XML_InterfaceType),     # <  \~chinese 节点类型         \~english Node Type
    ('enVisivility', MV_XML_Visibility),  # <  \~chinese 是否可见         \~english Is visibility
    ('strDescription', c_char * 512),     # < \~chinese 节点描述,目前暂不支持   \~english Node Description, NOT SUPPORT NOW
    ('strDisplayName', c_char * 64),      # < \~chinese 显示名称         \~english Display Name
    ('strName', c_char * 64),             # < \~chinese 节点名           \~english Node Name
    ('strToolTip', c_char * 512),         # < \~chinese 提示             \~english Notice
    ('nReserved', c_uint * 4),            # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_NODES_LIST_._fields_ = [
    ('nNodeNum', c_uint),                    # < \~chinese 节点个数             \~english Node Number
    ('stNodes', MV_XML_NODE_FEATURE * 128),  # < \~chinese 单个节点基本属性       \~english Node Basic Attributes
]
...
_MV_XML_FEATURE_Base_._fields_ = [
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
]
...
_MV_XML_FEATURE_Integer_._fields_ = [
    ...
    ('strDescription', c_char * 512),    # < \~chinese 节点描述,目前暂不支持   \~english Node Description, NOT SUPPORT NOW
    ('strToolTip', c_char * 512),        # < \~chinese 提示             \~english Notice
    ('enVisivility', MV_XML_Visibility),  # < \~chinese 是否可见         \~english Visible
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
    ('bIsLocked', c_int),                 # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('nValue', int64_t),                  # < \~chinese 当前值           \~english Current Value
    ('nMinValue', int64_t),               # < \~chinese 最小值           \~english Min Value
    ('nMaxValue', int64_t),               # < \~chinese 最大值           \~english Max Value
    ('nIncrement', int64_t),              # < \~chinese 增量             \~english Increment
    ('nReserved', c_uint * 4),            # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_FEATURE_Boolean_._fields_ = [
    ...
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
    ('bIsLocked', c_int),                 # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('bValue', c_bool),                   # < \~chinese 当前值           \~english Current Value
    ('nReserved', c_uint * 4),            # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_FEATURE_Float_._fields_ = [
    ...
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
    ('bIsLocked', c_int),                 # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('dfValue', c_double),                # < \~chinese 当前值           \~english Current Value
    ('dfMinValue', c_double),             # < \~chinese 最小值           \~english Min Value
    ('dfMaxValue', c_double),             # < \~chinese 最大值           \~english Max Value
    ('dfIncrement', c_double),            # < \~chinese 增量             \~english Increment
    ('nReserved', c_uint * 4),            # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_FEATURE_String_._fields_ = [
    ...
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
    ('bIsLocked', c_int),                 # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('strValue', c_char * 64),            # < \~chinese 当前值           \~english Current Value
    ('nReserved', c_uint * 4),            # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_FEATURE_Register_._fields_ = [
    ...
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
    ('bIsLocked', c_int),  # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('nAddrValue', int64_t),              # < \~chinese 当前值           \~english Current Value
    ('nReserved', c_uint * 4),            # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_FEATURE_EnumEntry_._fields_ = [
    ...
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式         \~english Access Mode
    ('bIsLocked', c_int),                 # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('nReserved', c_int * 8),             # < \~chinese 预留              \~english Reserved bytes
]
...
_MV_XML_FEATURE_Enumeration_._fields_ = [
    ...
    ('strName', c_char * 64),          # < \~chinese 节点名           \~english Node Name
    ('strToolTip', c_char * 512),      # < \~chinese 提示             \~english Notice
    ('nSymbolicNum', c_int),           # < \~chinese ymbolic数        \~english Symbolic Number
    ('strCurrentSymbolic', c_char * 64),  # < \~chinese 当前Symbolic索引    \~english Current Symbolic Index
    ('strSymbolic', c_char * 64 * 64),    # < \~chinese 当前Symbolic    \~english Current Symbolic
    ('enAccessMode', MV_XML_AccessMode),  # < \~chinese 访问模式        \~english Access Mode
    ('bIsLocked', c_int),  # < \~chinese 是否锁定。0-否；1-是,目前暂不支持    \~english Locked. 0-NO; 1-YES, NOT SUPPORT NOW
    ('nValue', int64_t),   # < \~chinese 当前值           \~english Current Value
    ('nReserved', c_uint * 4),  # < \~chinese 预留              \~english Reserved bytes
...
N23_MV_XML_CAMERA_FEATURE_3DOT_1E._fields_ = [
    ('stIntegerFeature', MV_XML_FEATURE_Integer),    # < \~chinese 整型属性       \~english integer feature
    ('stFloatFeature', MV_XML_FEATURE_Float),        # < \~chinese 浮点型属性       \~english float feature
    ('stEnumerationFeature', MV_XML_FEATURE_Enumeration),  # < \~chinese 枚举型属性       \~english enumeration feature
    ('stStringFeature', MV_XML_FEATURE_String),            # < \~chinese 字符型属性       \~english string feature
]
